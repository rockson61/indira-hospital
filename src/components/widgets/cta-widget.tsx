...
export function CTAWidget({
  ...
  return (
    <ModernCard className={`overflow-hidden relative ${urgency ? 'bg-gradient-to-br from-red-50 to-orange-50 dark:from-red-900/20 dark:to-orange-900/20 border-red-200 dark:border-red-800' : 'bg-gradient-to-br from-fuchsia-50 via-cyan-50 to-blue-50 dark:from-fuchsia-950/30 dark:via-cyan-950/30 dark:to-fuchsia-950/30 border-fuchsia-100 dark:border-fuchsia-800'}`}>
      {/* Background Decor */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-fuchsia-200/20 dark:bg-fuchsia-500/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2" />
      <div className="absolute bottom-0 left-0 w-64 h-64 bg-fuchsia-200/20 dark:bg-fuchsia-500/10 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2" />

      <div className="p-8 md:p-10 relative z-10">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex-1 text-center md:text-left space-y-4">
            <h3 className={`text-3xl md:text-4xl font-bold tracking-tight ${urgency ? 'text-red-600 dark:text-red-400' : 'bg-gradient-to-r from-fuchsia-700 to-fuchsia-700 dark:from-fuchsia-300 dark:to-fuchsia-300 bg-clip-text text-transparent'}`}>
              {title}
            </h3>
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-x-6 gap-y-2 mt-4">
                {benefits.map((benefit, index) => (
                  <div key={index} className="flex items-center text-sm font-medium text-slate-700 dark:text-slate-200">
                    <CheckCircle className="w-4 h-4 text-fuchsia-500 mr-2 flex-shrink-0" />
                    <span>{benefit}</span>
                  </div>
                ))}
              </div>
          ...
          <div className="flex flex-col w-full md:w-auto gap-4">
            <div className="flex flex-col sm:flex-row gap-3 w-full">
              <Button
                ...
                size="lg"
                className={`${urgency ? 'bg-red-600 hover:bg-red-700 shadow-red-200 dark:shadow-red-900/20' : 'bg-gradient-to-r from-fuchsia-600 to-fuchsia-600 hover:from-fuchsia-700 hover:to-fuchsia-700 shadow-fuchsia-200 dark:shadow-fuchsia-900/20'} text-white font-bold shadow-lg w-full sm:w-auto px-8 py-6 text-lg transition-transform hover:-translate-y-0.5`}
              >
