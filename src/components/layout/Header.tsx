...
export function Header() {
 ...
 <div 
 className={cn(
 "absolute inset-0 rounded-[2.5rem] transition-all duration-500 pointer-events-none border border-slate-200 dark:border-slate-800 dark:border-slate-700/60 shadow-clay",
 scrolled ? 'bg-white dark:bg-slate-900 ' : 'bg-white dark:bg-slate-900'
 )} 
 />
 ...
 <LanguageSwitcher />
 <ThemeToggle />
 <Link href="/book-appointment" className="elite-button-primary !px-6 !py-2.5 !text-[11px]">Book Consultation</Link>
 </div>
 ...
 {/* Mobile Menu Overlay */}
 <div className={`fixed inset-0 z-[90] bg-white dark:bg-slate-900 transition-transform duration-300 lg:hidden ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
 ...
 className={`flex items-center justify-between p-4 rounded-3xl text-lg font-bold transition-all ${pathname === item.href ? "bg-fuchsia-50 dark:bg-fuchsia-950 text-fuchsia-700" : "text-slate-800 dark:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800" }`}
 >
 {item.title}
