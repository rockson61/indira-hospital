...
export function SubServiceTemplate({
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      {/* ── Specialists Section (Full Width) ──────────────────────────── */}
      {displaySpecialists.length > 0 && (
        <section className="bg-slate-50 dark:bg-slate-950 py-20 border-t border-slate-100 dark:border-slate-800">
          <SectionContainer>
            <div className="flex flex-col md:flex-row items-end justify-between mb-12 gap-6">
              <div>
                ...
                <h2 className="elite-section-title text-slate-900 dark:text-white mt-2">
                  {departmentName ? `${departmentName} Specialists Near You` : 'Consult Our Senior Specialists'}
                </h2>
      ...
      <section className="bg-white dark:bg-slate-900 py-16 border-t border-slate-100 dark:border-slate-800">
        <SectionContainer>
          <div className="grid md:grid-cols-3 gap-8">
            {/* Book Appointment */}
            <ModernCard className="p-8 bg-white dark:bg-slate-800 shadow-float border border-slate-100 dark:border-slate-700">
              ...
              <div className="space-y-3">
                ...
                <a
                  ...
                  className="w-full inline-flex items-center justify-center px-6 py-4 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-full transition-all text-base"
                >
                  <MessageCircle className="w-5 h-5 mr-2" />
                  WhatsApp Now
                </a>
                <a
                  ...
                  className="w-full inline-flex items-center justify-center px-6 py-4 bg-slate-50 dark:bg-slate-700 hover:bg-slate-100 text-slate-700 dark:text-white font-bold rounded-full transition-all border border-slate-200 dark:border-slate-600 text-base"
                >
            ...
            {/* OPD Schedule */}
            <ModernCard className="p-8 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-clay-sm">
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                <Clock className="w-5 h-5 text-fuchsia-600" />
                OPD Schedule
              </h3>
              <div className="divide-y divide-slate-100 dark:divide-slate-700">
                {OPD_SCHEDULE.map((row) => (
                  <div key={row.day} className="flex justify-between items-center py-4">
                    <span className="text-slate-600 dark:text-slate-400 font-medium text-sm">{row.day}</span>
                    {row.emergency ? (
                      <span className="px-3 py-1 bg-red-100 text-red-600 text-xs font-bold rounded-full">ALWAYS OPEN</span>
                    ) : (
                      <span className="font-bold text-fuchsia-700 dark:text-fuchsia-400 text-sm">{row.time}</span>
                    )}
                  </div>
                ))}
              </div>
            ...
            {/* Why Indira */}
            <ModernCard className="p-8 bg-gradient-to-br from-slate-900 to-indigo-950 border-none shadow-float text-white">
              <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                <Shield className="w-5 h-5 text-fuchsia-400" />
                Why Indira Hospital?
              </h3>
              <ul className="space-y-4">
                {WHY_INDIRA_POINTS.map((item, i) => (
      ...
      {/* ── Related Services ──────────────────────────────────────────── */}
      {relatedServices && (
        <section id="related-treatments" className="bg-slate-50 dark:bg-slate-950 py-20 border-b border-slate-100 dark:border-slate-700">
          <SectionContainer>
