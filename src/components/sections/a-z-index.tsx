            ...
            <SectionContainer>
                <div className="text-center max-w-3xl mx-auto mb-16">
                    <h2 className="text-3xl md:text-5xl font-black mb-6 text-slate-900 dark:text-white tracking-tight">
                        Health <span className="text-teal-600">A to Z Index</span>
                    </h2>
                ...
                <div className="max-w-2xl mx-auto mb-12 relative group">
                    <Search className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-teal-600 transition-colors" />
                    <input
                        type="text"
                        placeholder="Search for a treatment or condition..."
                        className="w-full pl-14 pr-6 py-5 rounded-2xl bg-slate-50 dark:bg-slate-800 border-none outline-none focus:ring-2 focus:ring-teal-600 text-lg transition-all"
                        value={searchTerm}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {filteredData.map((item, i) => (
                        ...
                        >
                            <span className="font-bold text-slate-800 dark:text-slate-200 group-hover:text-teal-600 transition-colors">{item.name}</span>
