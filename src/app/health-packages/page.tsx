        ...
        <main className="min-h-screen bg-slate-50 selection:bg-teal-200 selection:text-teal-900 pb-24">
                ...
                <SectionContainer className="relative z-10 text-center">
                    <div className="max-w-4xl mx-auto">
                        ...
                        <h1 className="text-5xl sm:text-7xl font-black mb-6 tracking-tight leading-[1.1]">
                            ...
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-indigo-400">Screening Packages.</span>
                        </h1>
            ...
            <SectionContainer className="relative z-20 -mt-24 sm:-mt-32 max-w-7xl mx-auto px-4 mb-24">
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {packages.map((pkg) => (
                        <div key={pkg.slug} className={`group bg-white rounded-[2.5rem] border ${pkg.is_featured ? 'border-teal-400 shadow-[0_0_40px_-10px_rgba(20,184,166,0.3)]' : 'border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)]'} hover:shadow-[0_20px_40px_rgba(20,184,166,0.15)] hover:border-teal-300 hover:-translate-y-2 transition-all duration-500 flex flex-col relative overflow-hidden bg-clip-padding`}>
                            {/* Elevated Highlight Background */}
                            {pkg.is_featured && (
                                <div className="absolute inset-x-0 top-0 h-2 bg-gradient-to-r from-teal-400 via-emerald-400 to-teal-400" />
                            )}
                            ...
                            <div className="p-8 sm:p-10 pb-0 relative z-10">
                                ...
                                <h2 className="text-2xl sm:text-3xl font-black text-slate-900 mb-3 tracking-tight group-hover:text-teal-700 transition-colors">{pkg.title}</h2>
                                ...
                                <div className="flex items-baseline gap-3 mb-8">
                                    <span className="text-4xl font-black text-slate-900 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-teal-600 group-hover:to-emerald-500 transition-all">₹{pkg.price}</span>
                                    {pkg.original_price && (
                                ...
                                <div className="space-y-4 mb-10">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {pkg.tests_included.split(',').slice(0, 5).map((test, i) => (
                                            <span key={i} className="flex items-center gap-1.5 text-xs font-bold text-slate-600 bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-xl group-hover:border-teal-100 group-hover:bg-teal-50 shadow-sm transition-colors">
                            ...
                            <div className="mt-auto p-8 sm:p-10 pt-0 space-y-3 relative z-10 w-full">
                                ...
                                <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(`Hi, I'm interested in booking the ${pkg.title} package.`)}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-full px-6 py-4 bg-teal-50 text-teal-700 font-bold rounded-2xl hover:bg-teal-600 hover:text-white transition-all duration-300">
                ...
                {packages.length === 0 && (
                    <div className="text-center py-24 bg-white/50 backdrop-blur-xl rounded-[3rem] border-2 border-dashed border-slate-200 mt-12">
                        ...
                        <Link href="/contact" className="inline-flex px-8 py-4 bg-teal-600 text-white font-bold rounded-2xl hover:bg-teal-700 transition-all">
                            Contact Us
                        </Link>
                    </div>
                )}
            ...
            <SectionContainer className="pb-24">
                <div className="bg-gradient-to-br from-slate-900 via-indigo-950 to-teal-950 rounded-[4rem] border border-white/10 overflow-hidden shadow-2xl relative group/cta">
                    ...
                    <div className="grid lg:grid-cols-2 relative z-10">
                        <div className="p-10 md:p-16 lg:p-24 flex flex-col justify-center">
                            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-teal-300 text-xs font-bold tracking-widest uppercase mb-6 w-fit">
                                Clinical Excellence
                            </div>
                            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-10 text-white tracking-tight leading-[1.1]">
                                Why Health Checkups at <br className="hidden sm:block" /> <span className="text-teal-400">Indira Hospital?</span>
                            </h2>
                            <div className="space-y-8">
                                <div className="flex gap-6 items-start group">
                                    <div className="w-14 h-14 rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center flex-shrink-0 border border-white/10 group-hover:bg-teal-500 transition-colors duration-300">
                                        <Zap className="w-6 h-6 text-teal-300 group-hover:text-white" />
                                    </div>
                                    <div>
                                ...
                                <div className="flex gap-6 items-start group">
                                    <div className="w-14 h-14 rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center flex-shrink-0 border border-white/10 group-hover:bg-teal-500 transition-colors duration-300">
                                        <Heart className="w-6 h-6 text-teal-300 group-hover:text-white" />
                                    </div>
                                    <div>
