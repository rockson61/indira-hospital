...
export default function WhyChoosePage() {
 ...
 {values.map((value, i) => (
 <div key={i} className="group relative bg-white dark:bg-slate-900 p-10 rounded-[3.5rem] border border-slate-200 dark:border-slate-800 shadow-2xl transition-all duration-500 hover:-translate-y-2 overflow-hidden">
 ...
 <p className="text-slate-600 dark:text-subtle-on-light font-medium leading-relaxed">
 {value.description}
 </p>
 </div>
 ))}
 ...
 <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-fuchsia-500/10 rounded-full blur-[120px] will-change-transform transform-gpu opacity-50"/>
 <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-[0.03] pointer-events-none"/>
 
 <div className="relative z-10 grid lg:grid-cols-2 gap-20 items-center">
 <div>
 <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-fuchsia-300 text-sm font-black tracking-widest uppercase mb-8">
 ...
 <p className="text-subtle-on-light text-xl font-light leading-relaxed mb-12 max-w-xl">
 ...
 <div className="h-16 w-px bg-white dark:bg-slate-900 hidden sm:block mt-2"/>
 <div>
 ...
 <div key={i} className="aspect-square p-8 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[3rem] flex flex-col items-center justify-center gap-4 hover:bg-white dark:bg-slate-900 hover:scale-105 transition-all duration-500 group/item">
 <div className={`${item.color} group-hover/item:scale-110 transition-transform duration-500`}>
 {item.icon}
 </div>
 <span className="text-sm font-black text-white uppercase tracking-[0.2em] text-center">{item.label}</span>
 </div>
 ))}
 </div>
 </div>
 ...
 <InternalLinkGrid type="diagnostics"title="Diagnostic Excellence"subtitle="NABL Accredited Lab"limit={12} className="mt-16 bg-slate-50 dark:bg-slate-900 rounded-[4rem] border"/>
 </div>
 </main>
 );
}
