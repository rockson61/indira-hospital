            ...
            <section className="relative pt-36 pb-32 lg:pt-48 lg:pb-56 overflow-hidden bg-slate-900 rounded-b-[3rem] sm:rounded-b-[5rem]">
                ...
                <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-20 text-center">
                    ...
                    <h1 className="text-6xl md:text-8xl lg:text-[7rem] font-black text-white tracking-tight leading-[1.05] mb-8 max-w-5xl mx-auto">
                        Elite <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-emerald-300">Proficiency.</span>
                    </h1>
                    ...
                    <div className="max-w-4xl mx-auto bg-white/10 backdrop-blur-3xl p-3 sm:p-5 rounded-[2.5rem] border border-white/20 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.6)] flex flex-col lg:flex-row gap-4">
                        <div className="relative flex-grow group">
                            <div className="absolute inset-y-0 left-0 pl-8 flex items-center pointer-events-none">
                                <Search className="h-6 w-6 text-teal-400 group-focus-within:text-teal-300 transition-colors" />
                            </div>
                            <input
                                type="text"
                                placeholder="Search by name or specialty..."
                                className="block w-full pl-18 pr-6 py-5 sm:py-6 bg-white/5 text-white placeholder-slate-400 rounded-3xl focus:ring-4 focus:ring-teal-500/20 focus:bg-white/10 transition-all outline-none font-bold border border-transparent focus:border-white/30 text-xl"
                                value={searchQuery}
                        ...
                        <div className="relative min-w-[280px] group">
                            <div className="absolute inset-y-0 left-0 pl-8 flex items-center pointer-events-none z-10">
                                <Filter className="h-6 w-6 text-teal-400 group-focus-within:text-teal-300 transition-colors" />
                            </div>
                            <select
                                className="block w-full pl-18 pr-12 py-5 sm:py-6 bg-white/5 text-white rounded-3xl focus:ring-4 focus:ring-teal-500/20 focus:bg-slate-900 transition-all outline-none appearance-none font-bold border border-transparent focus:border-white/30 text-xl cursor-pointer"
                                value={selectedDept}
                            ...
                            <div className="absolute inset-y-0 right-0 pr-6 flex items-center pointer-events-none text-teal-400">
                    ...
                    {filteredDoctors.length === 0 ? (
                        ...
                        >
                            ...
                            <button
                                ...
                                className="group/btn relative px-12 py-6 bg-teal-500 text-slate-900 rounded-2xl font-black text-lg transition-all hover:scale-[1.05] shadow-xl shadow-teal-500/25 overflow-hidden"
                            >
                                ...
                                >
                                    <div className="relative h-full bg-white/70 backdrop-blur-2xl border border-slate-200/50 rounded-[3.5rem] p-10 lg:p-14 hover:bg-white hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.1)] hover:-translate-y-2 transition-all duration-700 flex flex-col overflow-hidden">
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-10 mb-10 items-start relative z-10">
                                            <div className="h-32 w-32 lg:h-40 lg:w-40 rounded-[2.5rem] bg-gradient-to-br from-teal-50 to-emerald-50 border border-teal-100 flex-shrink-0 flex items-center justify-center shadow-md group-hover:shadow-xl group-hover:scale-105 transition-all duration-700 relative overflow-hidden">
                                                <div className="absolute inset-0 bg-teal-500/5 opacity-0 group-hover:opacity-100 transition-opacity animate-pulse" />
                                                <span className="text-teal-700 font-black text-5xl tracking-tighter uppercase relative z-10">
                                                    {doctor.name.split(' ').map((n: string) => n[0]).join('').slice(0, 2)}
                                                </span>
                                            </div>
                                            <div className="pt-2">
                                                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-teal-50 border border-teal-100 text-[10px] font-black uppercase tracking-[0.2em] text-teal-600 mb-4 shadow-sm group-hover:bg-teal-500 group-hover:text-white transition-colors">
                                                    <Sparkles className="w-3 h-3" /> Specialist
                                                </div>
                                                <h2 className="text-4xl lg:text-5xl font-black text-slate-900 mb-3 group-hover:text-teal-700 transition-colors tracking-tight leading-[1.1]">
                                                ...
                                                <div className="space-y-1">
                                                    <p className="text-xl font-bold text-teal-600 group-hover:text-teal-500 transition-colors">{doctor.specialty}</p>
                                        ...
                                        <div className="grid grid-cols-2 gap-4 mb-12 relative z-10">
                                            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 group-hover:bg-white group-hover:border-teal-100 transition-all">
                                                <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">
                                                    <Clock className="w-3 h-3 text-teal-500" /> Experience
                                                </div>
                                                <p className="text-lg font-black text-slate-900">{doctor.experience}{DOCTORS_DIRECTORY.EXPERIENCE_SUFFIX}</p>
                                            </div>
                                            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 group-hover:bg-white group-hover:border-teal-100 transition-all">
                                                <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">
                                                    <Star className="w-3 h-3 text-emerald-500" /> Qualification
                                                </div>
                                        ...
                                        <div className="pt-10 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-6 relative z-20">
                                            <Link
                                                ...
                                                className="text-sm font-black text-slate-400 hover:text-teal-600 uppercase tracking-[0.2em] transition-all flex items-center gap-2"
                                            >
                                                View Profile <ArrowRight className="w-4 h-4" />
                                            </Link>
                                            <Link
                                                href={`${DOCTORS_DIRECTORY.BOOK_VISIT_HREF_PREFIX}?doctor=${doctor.id}`}
                                                className="group/btn relative inline-flex items-center justify-center w-full sm:w-auto px-10 py-5 bg-slate-900 text-white font-black rounded-2xl hover:bg-teal-500 hover:scale-[1.02] shadow-xl transition-all duration-500 overflow-hidden"
                                            >
