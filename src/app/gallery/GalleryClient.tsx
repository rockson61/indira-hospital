...
export default function GalleryClient({ images }: GalleryClientProps) {
 ...
 className={`px-8 py-3 rounded-full text-xs font-black uppercase tracking-[0.2em] transition-all duration-500 border ${ filter === cat ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 border-slate-900 dark:border-white shadow-2xl shadow-indigo-500/20 scale-105' : 'bg-white dark:bg-slate-900 text-slate-500 dark:text-subtle-on-light border-slate-200 dark:border-slate-800 hover:border-indigo-400 dark:hover:border-indigo-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:scale-105' }`}
 >
 {cat}
 </button>
 ))}
 ...
 <span className="text-xs font-black text-indigo-400 uppercase tracking-[0.3em] mb-3">
 {photo.category}
 </span>
 ...
 <span className="inline-flex items-center px-5 py-2 rounded-full bg-black/40 text-white text-xs font-black uppercase tracking-[0.2em] border border-slate-200 dark:border-slate-800 shadow-2xl">
 {photo.category}
 </span>
 </div>
