                ...
                {categories.map(cat => (
                    <button
                        ...
                        className={`px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-500 border ${
                            ...
                            : 'bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:border-indigo-400 dark:hover:border-indigo-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:scale-105'
                        }`}
                    >
                        {cat}
                    </button>
                    ...
                    {filteredPhotos.map((photo, index) => (
                        ...
                        >
                            ...
                            <div className="absolute top-8 left-8 z-20">
                                <span className="inline-flex items-center px-5 py-2 rounded-full bg-black/40 backdrop-blur-2xl text-white text-[10px] font-black uppercase tracking-[0.2em] border border-white/10 shadow-2xl">
                                    {photo.category}
                                </span>
                            </div>
            ...
            <AnimatePresence>
                {selectedImage && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-10 lg:p-20 bg-slate-950/98 backdrop-blur-3xl"
                        onClick={() => setSelectedImage(null)}
                    >
                        <motion.button 
                            className="absolute top-8 right-8 w-14 h-14 bg-white/10 hover:bg-white/20 hover:scale-110 active:scale-90 transition-all rounded-full flex items-center justify-center text-white z-[110] backdrop-blur-xl border border-white/20"
                            onClick={() => setSelectedImage(null)}
                        >
                            <X className="w-8 h-8" />
                        </motion.button>
